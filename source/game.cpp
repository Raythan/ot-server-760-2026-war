						 ...
						 )
{
	...
	if (!p || c->access >= g_config.ACCESS_ENTER || getPlayersOnline() < max_players) {
		...
		if(success) {
			...
			if(p){
				c->eventCheck = addEvent(makeTask(1000, boost::bind(&Game::checkCreature, boost::placeholders::_1, c->getID())));
			}
			else{
				c->eventCheck = addEvent(makeTask(500, boost::bind(&Game::checkCreature, boost::placeholders::_1, c->getID())));
			}

			//c->eventCheckAttacking = addEvent(makeTask(2000, boost::bind(&Game::checkCreatureAttacking, boost::placeholders::_1, c->getID())));
		}
	}
	else {
		...
		if (attackedCreature->manaShieldTicks < 1000 && (creatureState.damage == 0) &&
			...
			for(std::vector<Creature*>::const_iterator cit = creatureState.attackerlist.begin(); cit != creatureState.attackerlist.end(); ++cit) {
				...
				if(spectator->CanSee(gainexpCreature->pos.x, gainexpCreature->pos.y, gainexpCreature->pos.z)) {
					...
					sprintf(exp, "%lld", (long long)attackedCreature->getGainedExperience(gainexpCreature));
#else
...
#endif
*/
		player->eventAutoWalk = addEvent(makeTask(ticks, boost::bind(&Game::checkPlayerWalk, boost::placeholders::_1, id)));
	}
	else
...
void Game::checkCreature(unsigned long id)
{
	...
	if (creature && creature->isRemoved == false)
	{
		...
		if(thinkTicks > 0) {
			creature->eventCheck = addEvent(makeTask(thinkTicks, boost::bind(&Game::checkCreature, boost::placeholders::_1, id)));
		}
		else
		...
		if (monster) {
          ...
          default:
           ...
           break;                            
                ...
                creature->eventCheckAttacking = addEvent(makeTask(speed, boost::bind(&Game::checkCreatureAttacking, boost::placeholders::_1, id)));
				}
			}
		}
		flushSendBuffers();
...
void Game::checkSpawns(int t)
{
	...
	this->addEvent(makeTask(t, boost::bind(&Game::checkSpawns, boost::placeholders::_1, t)));
}
...
*/
	...
	player->eventAutoWalk = addEvent(makeTask(ticks, boost::bind(&Game::checkPlayerWalk, boost::placeholders::_1, player->getID())));
...
void Game::playerSetAttackedCreature(Player* player, unsigned long creatureid)
{
	...
	if(attackedCreature) {
		...
		stopEvent(player->eventCheckAttacking);
		player->eventCheckAttacking = addEvent(makeTask(500, boost::bind(&Game::checkCreatureAttacking, boost::placeholders::_1, player->getID())));
	}
...
void Game::autoServerSave()
{
	...
	serverSave();
	addEvent(makeTask(g_config.getGlobalNumber("autosave", 1)*60000, boost::bind(&Game::autoServerSave, boost::placeholders::_1)));
}
