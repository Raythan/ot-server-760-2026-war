...
{
  if (luaState) {
	  if (globalsTableRef != LUA_NOREF)
		  luaL_unref(luaState, LUA_REGISTRYINDEX, globalsTableRef);
	  lua_close(luaState);
  }
}
...
int LuaScript::OpenFile(const char *filename)
{
	...
	luaState = lua_open();
	/* Lua 5.1: state must have standard libs opened or globals/env API can panic
	   ("no calling environment") when reading config after dofile. */
	...
	globalsTableRef = luaL_ref(luaState, LUA_REGISTRYINDEX);
...
{
	if (!luaState || globalsTableRef == LUA_NOREF)
		return defString;
	lua_rawgeti(luaState, LUA_REGISTRYINDEX, globalsTableRef);
	...
	std::string ret = s ? std::string(s) : defString;
	lua_pop(luaState, 2);
...
int LuaScript::setGlobalNumber(std::string var, lua_Number val){
	if (!luaState)
		return false;
	if (globalsTableRef != LUA_NOREF) {
		lua_rawgeti(luaState, LUA_REGISTRYINDEX, globalsTableRef);
		...
		/* SpellScript / ActionScript: no registry snapshot; write into global env */
		lua_pushnumber(luaState, val);
		lua_setfield(luaState, LUA_GLOBALSINDEX, var.c_str());
	}
	return true;
}
...
std::string LuaScript::getGlobalStringField (std::string var, const int key, const std::string &defString) {
	...
	const char* s = lua_tostring(luaState, -1);
	std::string result = s ? std::string(s) : defString;
	lua_pop(luaState, 3);
	return result;
}
