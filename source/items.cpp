...
{
	ItemLoader f;
	/* Caching reads the file in blocks; keep it off for OTB to avoid subtle heap issues. */
	if(!f.openFile(file.c_str(), false, false)){
		return f.getError();
	}
	...
	while(node != NO_NODE) {
		...
		if(data != NULL) {
			...
			std::unique_ptr<ItemType> iTypeHolder(new ItemType());
			ItemType* iType = iTypeHolder.get();
			...
			while(p < data + len) {
				...
				switch(type) {
					...
					case ITEM_GROUP_FLUID:
					{
						...
						{
							ptrdiff_t room = (data + len) - p;
							if(room < 0 || (size_t)room < sizeof(datasize_t))
								return ERROR_INVALID_FORMAT;
						}
						...
						{
							ptrdiff_t rem = (data + len) - p;
							if(rem < 0 || (ptrdiff_t)datalen > rem)
								return ERROR_INVALID_FORMAT;
						}
