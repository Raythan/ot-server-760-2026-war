...
static const char* fileLoaderErrorStr(int e)
{
	switch(e) {
		case ERROR_NONE: return "none";
		case ERROR_INVALID_FILE_VERSION: return "invalid file version";
		case ERROR_CAN_NOT_OPEN: return "cannot open file (missing path or no read permission)";
		case ERROR_CAN_NOT_CREATE: return "cannot create file";
		case ERROR_EOF: return "unexpected end of file";
		case ERROR_SEEK_ERROR: return "seek error";
		case ERROR_NOT_OPEN: return "file not open";
		case ERROR_INVALID_NODE: return "invalid node";
		case ERROR_INVALID_FORMAT: return "invalid format (or unsupported OTB client version — need 7.60)";
		case ERROR_TELL_ERROR: return "tell error";
		case ERROR_COULDNOTWRITE: return "write error";
		case ERROR_CACHE_ERROR: return "cache error";
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
	...
	// read global config (cwd first; under _HOMEDIR_CONF_ also try ~/.otserv/)
	std::cout << ":: Loading lua script config.lua... ";
	bool configLoaded = g_config.OpenFile("config.lua");
#ifdef _HOMEDIR_CONF_
	if (!configLoaded && getenv("HOME")) {
		std::string configpath = getenv("HOME");
		configpath += "/.otserv/config.lua";
		configLoaded = g_config.OpenFile(configpath.c_str());
	}
#endif
	if (!configLoaded)
	{
	...
	std::string itemsOtbPath = g_config.getGlobalString("datadir") + "items/items.otb";
	int otbErr = Item::items.loadFromOtb(itemsOtbPath);
	if (otbErr != ERROR_NONE)
	{
		std::cout << std::endl;
		std::cout << "items.otb path: " << itemsOtbPath << std::endl;
		std::cout << "Load error (" << otbErr << "): " << fileLoaderErrorStr(otbErr) << std::endl;
		if (otbErr == ERROR_CAN_NOT_OPEN)
			std::cout << "This repository does not ship items.otb; copy a Tibia 7.60-compatible items.otb into data/items/." << std::endl;
		else if (otbErr == ERROR_INVALID_FILE_VERSION)
			std::cout << "The file exists but the OTB header is invalid for this server (use a proper items.otb, not items.xml or another version)." << std::endl;
	...
	if (g_config.getGlobalNumber("autosave") > 0)
		g_game.addEvent(makeTask(g_config.getGlobalNumber("autosave")*60000, boost::bind(&Game::autoServerSave, boost::placeholders::_1)));
	else
