...
void Monster::reThink(bool updateOnlyState /* = true*/)
{
	if(isSummon()) {
		...
		//change target
		if(state != STATE_IDLE) {
			if(mType->changeTargetChance > static_cast<int>((static_cast<long long>(rand()) * 10000) / (static_cast<long long>(RAND_MAX) + 1))){
				bool canReach;
...
void Monster::startThink()
{
	...
	if(!eventCheck){
		eventCheck = game->addEvent(makeTask(500, boost::bind(&Game::checkCreature, boost::placeholders::_1, getID())));
	}

	if(attackedCreature != 0 && !(isSummon() && hasLostMaster)) {
		if(!eventCheckAttacking){
			eventCheckAttacking = game->addEvent(makeTask(500, boost::bind(&Game::checkCreatureAttacking, boost::placeholders::_1, getID())));
		}
	}
}
...
{
	if (attackedCreature != 0 && !(isSummon() && hasLostMaster)) {
		this->eventCheckAttacking = game->addEvent(makeTask(500, boost::bind(&Game::checkCreatureAttacking, boost::placeholders::_1, getID())));
